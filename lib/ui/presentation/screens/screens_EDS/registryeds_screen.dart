...
class _RegistryEDSState extends State<RegistryEDS> {
  ...
  final _ctrlBarrio = TextEditingController();
  final _ctrlValorG = TextEditingController();
  ...
  Future<void> _guardarEDS() async {
    if (!_formKey.currentState!.validate()) return;

    final nombre = _ctrlNombre.text;
    final barrio = _ctrlBarrio.text;
    final galon = int.parse(_ctrlValorG.text.replaceAll(',', ''));

    await _bd.addEDS(nombre, barrio, galon);

    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('EDS guardada correctamente'),
